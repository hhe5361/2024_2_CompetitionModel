    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            if phase == 'val' and epoch_acc > best_acc:
                best_acc = epoch_acc
                torch.save(model.state_dict(), 'res34_best.pt')
                best_model_wts = copy.deepcopy(model.state_dict())

            torch.save(model.state_dict(),'res34_latest.pt')
...
def evaluate(model,criterion, test_loader, device, ResNet18):
    ...
    #저장경로는 변경하셔도 됩니다.
    test_model.load_state_dict(torch.load('res34_best.pt'))
